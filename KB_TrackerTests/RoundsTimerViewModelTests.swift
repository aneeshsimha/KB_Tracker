// RoundsTimerViewModelTests.swift
// KB_TrackerTests
//
// Ported from TimerViewModelRoundsTests.swift — tests the new RoundsTimerViewModel.
...
@MainActor
struct RoundsTimerViewModelTests {
    ...
    /// setDone() with rounds remaining transitions to .resting and
    ...
    @Test func restCountdown_beepsAtFiveToOne() {
        ...
        //   call 3: setDone → setTime = now()-setStartTime (= 0, same epoch offset as we fix below)
        //   call 4: restStartTime = now() in setDone
        ...
        //   3: setDone setTime              → epoch+12 (0s set time, same as start)
    ...
    /// setDone() on the last round transitions to .complete, plays
