// TimerViewModelEMOMTests.swift
// KB_TrackerTests
//
// Characterization tests for TimerViewModel in EMOM mode.
...
@MainActor
private func driveEMOMGetReadyToActive(_ vm: TimerViewModel) {
    for _ in 0..<51 { vm.tick() }
}

@MainActor
struct TimerViewModelEMOMTests {
    ...
    @Test func getReadyCountdown_fiveBeepsThenGoBeepThenActive() {
        ...
        let vm = TimerViewModel(config: config, audio: spy)
    ...
    /// handleEMOMSetDone() appends now()-setStartTime to setTimes.
    /// We inject a fake clock: call 0 → start(), call 1 → startNewEMOMRound() (setStartTime),
    /// call 2 → handleEMOMSetDone() (now()).  Delta = 30 s.
    @Test func setDoneRecordsSetTimeFromInjectedClock() {
        ...
        //   call 2: handleEMOMSetDone() → now() = epoch+30 → setDuration = 30
    ...
    @Test func finalRoundCompletion_emomComplete() {
        ...
        let epoch = Date(timeIntervalSince1970: 1_000_000)
        var callIdx = 0
        // We need enough calls: start(), then startNewEMOMRound() x3, handleEMOMSetDone() x3
        let fakeNow: () -> Date = {
            defer { callIdx += 1 }
        ...
        // Round 3: mark set done — this triggers completeWorkout() via handleEMOMSetDone
    ...
    /// NOTE: pinning as-is — fix target in ANS-144.
    /// handleEMOMSetDone() also completes the workout when currentRound >= targetMinutes,
